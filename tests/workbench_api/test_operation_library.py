...
from operation_library import legacy_catalog_view, load_shared_operation_documents, load_workspace_operation_records


def write_operation(root: Path, workspace: str, filename: str, document: dict) -> None:
    directory = root / workspace / "operations"
...
def test_workspace_operations_extend_and_override_defaults(tmp_path: Path) -> None:
    ...
    records = load_workspace_operation_records(
...
def test_legacy_catalog_is_derived_from_filesystem_documents(tmp_path: Path) -> None:
    ...
    documents = load_shared_operation_documents(tmp_path)
