...
from workflow_operation_editor import ensure_example, load_example
from workflow_operations import (
    DEFAULT_DATATYPE_PATH,
    DEFAULT_OPERATION_PATH,
    OperationAwareWorkflowRouter,
...
def test_operation_catalog_has_workbench_and_arc3_demonstration_operations() -> None:
    raw = json.loads(DEFAULT_OPERATION_PATH.read_text(encoding="utf-8"))
    operations = {item["id"]: item for item in raw["operations"]}
    ...
    routes = {item["id"] for item in operations["grab_image_source"]["implementations"]}
    assert {
        "arc3_state",
        "video_to_frames",
        "ask_user_to_upload",
    ...
    assert "turtlized_objects_to_images" in operations
    assert "images_displayer" in operations
    species = {
...
def test_typed_example_uses_the_noninteractive_artifact_operations() -> None:
    operation_raw = json.loads(DEFAULT_OPERATION_PATH.read_text(encoding="utf-8"))
    operation_ids = {item["id"] for item in operation_raw["operations"]}
    example = load_example()
    example_operation_ids = {step["operation"] for step in example["steps"]}
    assert example_operation_ids.issubset(operation_ids)
    assert len(example_operation_ids) == 11
    assert example["steps"][0]["operation"] == "grab_image_source"


def test_operation_router_preprocesses_typed_steps(tmp_path, monkeypatch) -> None:
    ...
    router = OperationAwareWorkflowRouter(DEFAULT_CONFIG_PATH, workflow_path=path)
    workflow = router.workflow_by_id["typed_routed_artifact_pipeline"]
    assert len(router.operations) == 15
    assert len(workflow.steps) == 11
    assert all(
        step.transaction_id in router.operation_step_by_transaction_id
    ...
    raw = [
        {
            ...
            "steps": [
                {
                    "id": "extract",
                    "operation": "extract_individual_objects",
...
def test_arc3_human_observation_is_runnable_after_subworkflow_expansion(
    ...
    monkeypatch.setenv("ARC3_OPENROUTER_VERIFY_MODELS", "0")
    router = OperationAwareWorkflowRouter(DEFAULT_CONFIG_PATH)
