from pathlib import Path
import sys


ROOT = Path(__file__).resolve().parents[2]
SERVER = ROOT / "python" / "workbench_api_server"
if str(SERVER) not in sys.path:
    sys.path.insert(0, str(SERVER))

from workspace_api import _load_systems  # noqa: E402


def test_shared_callable_systems_are_not_model_backends() -> None:
    shared = ROOT / "workspaces" / "shared_library_system"
    records = _load_systems({"id": "shared_library_system", "root": str(shared)})
    documents = {record["document"]["id"]: record["document"] for record in records}

    assert {"python", "prolog", "metta", "llm", "omegaclaw", "codex", "mailbox"} <= documents.keys()
    assert all(document["kind"] == "system" for document in documents.values())
    assert documents["llm"]["systemType"] == "llm_caller"
    assert documents["mailbox"]["systemType"] == "communication"
    assert documents["mailbox"]["configuration"]["localIdentity"] == "symbolic-workbench-codex"
    assert "agent.message.send" in documents["mailbox"]["capabilities"]
    assert "agent.message.receive" in documents["mailbox"]["capabilities"]
    assert not ({"openai", "anthropic", "openrouter", "groq"} & documents.keys())
