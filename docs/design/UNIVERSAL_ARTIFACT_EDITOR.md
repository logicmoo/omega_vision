...
Resource and Inheritance are one content-backed tab. The selected resource remains directly editable there and saving an inherited resource creates a workspace-local override rather than modifying Shared. A separate collapsed preview shows resolved inheritance metadata. Enable/disable actions must offer an explicit scope chooser for the selected resource, its `dependsOn` dependencies, and its `dependedOnBy` dependents; enabling a resource that is blocked by a disabled dependency preselects the dependency scope. The editor identifies the effective inheritance parent and links to that real parent resource for direct editing, but that parent does not affect availability unless separately declared as a dependency.
