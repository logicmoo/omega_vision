...
def validate_write(runner,path,raw):
    ...
    try:OperationAwareWorkflowRouter(base,workflow_path=path)
...
def gui(runner,path,raw):
    ...
    catalog=read_obj(DEFAULT_OPERATION_PATH);operations={x["id"]:x for x in catalog["operations"]};operation_ids=list(operations);subworkflow_ids=[txt for txt in (str(x.get("id") or "") for x in workflows) if txt]
    ...
    ttk.Label(root,text="Compose typed operations and reusable subworkflows, then run the selected world-analysis workflow.").pack(anchor="w",padx=10,pady=(10,3))
    tabs=ttk.Notebook(root);tabs.pack(fill="both",expand=True,padx=10)
    wf_tab=ttk.Frame(tabs);operation_tab=ttk.Frame(tabs);dt_tab=ttk.Frame(tabs)
    tabs.add(wf_tab,text="Workflows");tabs.add(operation_tab,text="Operations / implementations");tabs.add(dt_tab,text="Datatype manifest")
    ...
    for c,h,w in zip(cols,("#","Item ID","Type","Operation / transaction","Implementation","Input slots","Output slots","Optional"),widths):stree.heading(c,text=h);stree.column(c,width=w)
    ...
    def summary(s):
        ...
        if s.get("operation"):
            ins=", ".join(f"{a}←{b}" for a,b in (s.get("inputs") or {}).items());outs=", ".join(f"{a}→{b}" for a,b in (s.get("outputs") or {}).items())
            return ("operation",s.get("operation",""),s.get("implementation",""),ins,outs)
    ...
    def operation_dialog(step,title):
        d=tk.Toplevel(root);d.title(title);d.transient(root);d.grab_set();d.columnconfigure(1,weight=1)
        v_id=tk.StringVar(value=step.get("id",""));v_operation=tk.StringVar(value=step.get("operation",operation_ids[0]));v_impl=tk.StringVar(value=step.get("implementation",""));v_profile=tk.StringVar(value=step.get("profile",""));v_model=tk.StringVar(value=step.get("model",""));v_level=tk.StringVar(value=str(step.get("analysis_level","")));v_opt=tk.BooleanVar(value=bool(step.get("continue_on_error")))
        for r,(text,var) in enumerate((("Item ID",v_id),("Operation",v_operation),("Implementation",v_impl))):ttk.Label(d,text=text).grid(row=r,column=0,sticky="w",padx=6,pady=3);(ttk.Entry(d,textvariable=var) if r==0 else ttk.Combobox(d,textvariable=var,width=50)).grid(row=r,column=1,sticky="ew",padx=6,pady=3)
        operation_box=d.grid_slaves(row=1,column=1)[0];operation_box.configure(values=operation_ids);impl_box=d.grid_slaves(row=2,column=1)[0]
        def update_impl(*_):
            vals=[x["id"] for x in operations[v_operation.get()]["implementations"]];impl_box.configure(values=vals)
            if v_impl.get() not in vals:v_impl.set(vals[0])
        operation_box.bind("<<ComboboxSelected>>",update_impl);update_impl()
        texts={}
        ...
        def accept():
            ...
            step.clear();step.update({"id":v_id.get().strip(),"operation":v_operation.get().strip(),"implementation":v_impl.get().strip(),**maps})
    ...
    def add_operation():
        ...
        s={"id":f"operation_{len(current().setdefault('steps',[]))+1}","operation":operation_ids[0],"implementation":operations[operation_ids[0]]["implementations"][0]["id"],"inputs":{},"outputs":{},"parameters":{}}
        if operation_dialog(s,"Add typed operation"):current()["steps"].append(s);refresh_s()
    ...
    def edit_operation():
        ...
        if s.get("subworkflow"):
            ...
            return
        if not s.get("operation"):messagebox.showinfo("Legacy transaction","Use Edit Raw JSON for legacy transaction items.");return
        if operation_dialog(s,"Edit typed operation"):refresh_s();stree.selection_set(str(i))
    def delete_step():
        i=step_index()
    ...
    wtree.bind("<<TreeviewSelect>>",load);stree.bind("<Double-1>",lambda _:edit_operation())
    ...
    for text,cmd in (("Add Operation",add_operation),("Add Subworkflow",add_subworkflow),("Edit Item",edit_operation),("Delete",delete_step),("Move Up",lambda:move(-1)),("Move Down",lambda:move(1))):ttk.Button(sbuttons,text=text,command=cmd).pack(side="left",padx=2)
    operation_tab.columnconfigure(0,weight=1);operation_tab.rowconfigure(0,weight=1);tt=ttk.Treeview(operation_tab,columns=("operation","ports","routes"),show="headings");tt.heading("operation",text="Operation");tt.heading("ports",text="Typed ports");tt.heading("routes",text="Implementation species / routes");tt.column("operation",width=250);tt.column("ports",width=550);tt.column("routes",width=700);tt.grid(row=0,column=0,sticky="nsew",padx=8,pady=8)
    for t in operations.values():tt.insert("","end",values=(t["id"],", ".join([*[f"in {k}:{v}" for k,v in t.get("inputs",{}).items()],*[f"out {k}:{v}" for k,v in t.get("outputs",{}).items()]]),", ".join(f"{x['id']} [{x['species']}]" for x in t["implementations"])))
    ...
    db=ttk.Frame(dt_tab);db.grid(row=1,column=0,sticky="ew",padx=8,pady=5);ttk.Button(db,text="Open Datatype Graph",command=lambda:open_path(GRAPH)).pack(side="left");ttk.Button(db,text="Open Manifest",command=lambda:open_path(DEFAULT_DATATYPE_PATH)).pack(side="left",padx=4);ttk.Button(db,text="Open Operation Catalog",command=lambda:open_path(DEFAULT_OPERATION_PATH)).pack(side="left")
    def save(show=True):
...
def open_operation_editor(runner):
    r=runner.llm_router()
    if not isinstance(r,OperationAwareWorkflowRouter):raise RuntimeError("Typed operation router not installed")
...
def install_workflow_operation_editor_ui(ui):
    if getattr(ui.read_key,"_arc3_operation_editor",False):return
    ...
    def read_key():
        ...
        r=last_runner()
        try:open_operation_editor(r) if r else print("No active ARC3 runner.")
        except Exception as e:print(f"Typed workflow editor error: {e}")
        return "\r"
    def help(r,rows):old_help(r,rows);print("Typed workflows: (W) operations + typed slots + LLM/Prolog/Python routes + datatype graph")
    read_key._arc3_operation_editor=True;ui.read_key=read_key;ui.print_controls=help
