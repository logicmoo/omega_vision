...
def hook_debugger(
    ...
    wait_for_user_if_not_started: bool = True,
) -> bool:
    try:
    ...
    while True:
        ...
        while time.monotonic() < deadline:
            try:
                # Suppress pydevd's large connection-refused traceback.
        ...
        while True:
            ...
            # Enter or C: continue normally.
            if response in {"", "c", "continue"}:
                print("Continuing without the PyCharm debugger.")
                return False

            # Q: terminate the entire program.
            if response in {"q", "quit", "exit"}:
                print("Exiting.")
                raise SystemExit(1)

            # T: retry using the current port.
            if response in {"t", "try", "retry"}:
                break

            # A number: use it as the new port and retry.
            try:
...
def read_key() -> str:
    ...
    if os.name == "nt":
        ...
        while True:
            ...
            if not ok:
                #print(ctypes.get_last_error(), "ReadConsoleInputW failed")
                #time.sleep(1)
