...
import {ResourceRelationshipForest} from "./ResourceRelationshipForest";
...
export function OperationLibraryEditor({workspaceId,sourceLanguage}:{workspaceId:string;sourceLanguage?:SourceLanguage}){
 ...
 const implementationsByResource=useMemo(()=>{const map=new Map<string,RecordFile<OperationImplementationDef>[]>();for(const row of implementations){for(const id of relationshipIds(row.document?.implements)){const list=map.get(id)||[];list.push(row);map.set(id,list)}}return map},[implementations]);
 ...
 const createImplementation=(doc:OpenDocument,parent:OperationResource)=>{const ids=new Set([...(snapshot?.operations||[]),...(snapshot?.operationImplementations||[])].flatMap(record=>record.document?[record.document.id]:[]));for(const opened of openDocs){try{const parsed=JSON.parse(opened.source) as OperationResource;if(parsed.id)ids.add(parsed.id)}catch{/* Invalid drafts do not contribute an id. */}}const stem=`${parent.id}.implementation`;let id=stem,index=2;while(ids.has(id)){id=`${stem}_${index++}`}const implementation=sourceLanguage==="prolog"?"prolog.source":sourceLanguage==="metta"?"metta.source":"python.callable";const implementationResource:OperationImplementationDef={kind:"operation",id,implements:implementsResource(parent.id),inheritsFrom:inheritsFromResource(parent.id),label:`${parent.label||parent.id} — Implementation`,description:"Concrete Operation implementation.",categories:parent.categories,topics:parent.topics,enabled:true,implementation};const implementedBy={...((parent as OperationDef).implementedBy||{}),...implementedByResource(id)};updateSource(doc.key,JSON.stringify({...parent,implementedBy,preferredImplementation:(parent as OperationDef).preferredImplementation||id},null,2));open({path:`design/operations/${slug(id)}.operation.json`,source:workspaceId==="shared"?"shared":"workspace",workspaceId,document:implementationResource,isNew:true})};
 ...
 const renderEditor=(doc:OpenDocument,secondary=false)=>{let document:OperationResource|null=null;try{document=doc.source?JSON.parse(doc.source) as OperationResource:null}catch{document=null}const isImplementation=Boolean(document&&relationshipIds(document.implements).length);const abstract=document&&!isImplementation?document as OperationDef:null;const selectedImplementation=document&&isImplementation?document as OperationImplementationDef:null;const implementedOperation=selectedImplementation?(snapshot?.operations||[]).find(row=>row.document&&relationshipIds(selectedImplementation.implements).includes(row.document.id))?.document:null;const variants=abstract?(implementationsByResource.get(abstract.id)||[]):[];const relatedById=new Map<string,OperationResource>();for(const record of [...(snapshot?.operations||[]),...(snapshot?.operationImplementations||[])])if(record.document)relatedById.set(record.document.id,record.document);for(const opened of openDocs){try{const parsed=JSON.parse(opened.source) as OperationResource;if(parsed.id)relatedById.set(parsed.id,parsed)}catch{/* Invalid drafts are excluded from derived inheritance. */}}const control:OperationSuperControlRequest={
  ...
  onCreateImplementation:document?()=>createImplementation(doc,document):undefined,
 };
 ...
 const commandBranches=(action:"collapse"|"expand",target:string)=>{if(target==="all"){commandCategories(action);commandOperations(action);return}if(target==="categories"){commandCategories(action);return}const rows=(snapshot.operations||[]).filter(row=>{const item=row.document;if(!item)return false;const variants=implementationsByResource.get(item.id)||[];if(target==="top-operation")return true;if(target==="unspecialized-operation")return variants.length===0;if(target==="enabled")return resolveResourceEnablement(item).enabled;if(target==="disabled")return !resolveResourceEnablement(item).enabled;if(target==="search")return JSON.stringify(item).toLowerCase().includes(treeFilter.trim().toLowerCase());return false});commandOperations(action,rows.flatMap(row=>row.document?[row.document.id]:[]))};
 const categorizedOperations=(snapshot.operations||[]).flatMap(operation=>{const item=operation.document;if(!item)return[];const variants=implementationsByResource.get(item.id)||[];const parentEnablement=resolveResourceEnablement(item);const categories=[...new Set([...categoryPaths(item.categories),...variants.flatMap(variant=>categoryPaths(variant.document?.categories))])];return [{id:item.id,categories,searchValue:item,render:(appearanceKey:string)=>{const selectedOperation=active?.record.document?.id===item.id;const branchCollapsed=collapsedOperations.has(item.id);const categoryPath=appearanceCategoryPath(appearanceKey,item.id);const operationBelongsHere=categoryPath!==null&&categoryPaths(item.categories).includes(categoryPath);const visibleVariants=categoryPath&&!operationBelongsHere?variants.filter(variant=>categoryPaths(variant.document?.categories).includes(categoryPath)):variants;return <div className={`operation-tree-group ${branchCollapsed?"branch-collapsed":""}`} key={appearanceKey} data-tree-search={JSON.stringify(item)}>
  <div className="inheritance-row"><button className={`operation-tree-row operation-parent ${enablementClass(parentEnablement)} ${selectedOperation?"selected":""}`} onClick={()=>open(operation as RecordFile<OperationResource>)}><span className="operation-kind-badge">OPERATION</span><span><b>{item.label||item.id}</b><small>{item.description||item.id}</small></span><em>{visibleVariants.length} implementations <ResourceEnablementBadge state={parentEnablement}/></em></button>{visibleVariants.length>0&&<button type="button" className="tree-branch-toggle" title={branchCollapsed?"Unhide Implementations":"Hide Implementations"} aria-label={`${branchCollapsed?"Unhide":"Hide"} Implementations for ${item.label||item.id}`} aria-expanded={!branchCollapsed} onClick={()=>toggleOperation(item.id)}><span aria-hidden="true">{branchCollapsed?"›":"⌄"}</span><b>{branchCollapsed?"Unhide Implementations":"Hide Implementations"}</b></button>}</div>
  <div className="operation-tree-children">{visibleVariants.map(variant=>{const impl=variant.document!;const implEnablement=resolveResourceEnablement(impl);const selectedImpl=active?.record.document?.id===impl.id;const implName=String(impl.implementation||"");const language=implName.startsWith("python")?"PYTHON":implName.startsWith("prolog")?"PROLOG":implName.startsWith("metta")?"METTA":implName.startsWith("llm")?"LLM":"IMPL";return <button className={`operation-tree-row operation-child ${enablementClass(implEnablement)} ${selectedImpl?"selected":""}`} key={impl.id} data-tree-search={JSON.stringify(impl)} onClick={()=>open(variant as RecordFile<OperationResource>)}><span className={`operation-kind-badge ${language.toLowerCase()}`}>{language}</span><span><b>{impl.label||impl.id}</b><small>{impl.implementation}</small></span><em>{item.preferredImplementation===impl.id?"default ":""}<ResourceEnablementBadge state={implEnablement}/></em></button>})}</div>
 </div>}}]});
 const relationshipForest=(relationshipMode:TreeRelationshipMode)=><ResourceRelationshipForest records={[...visibleOperations,...implementations]} relationshipMode={relationshipMode} renderNode={({record,depth,childCount})=>{const item=record.document!,state=resolveResourceEnablement(item);return <button className={`operation-tree-row ${depth?"operation-child":"operation-parent"} ${enablementClass(state)} ${active?.record.document?.id===item.id?"selected":""}`} onClick={()=>open(record as RecordFile<OperationResource>)}><span className="operation-kind-badge">{depth?"OP":"OPERATION"}</span><span><b>{item.label||item.id}</b><small>{item.description||item.id}</small></span><em>{childCount} {relationshipMode==="inheritance"?"inheritors":"dependents"} <ResourceEnablementBadge state={state}/></em></button>}}/>;
 return <section className="resource-view operation-hierarchy-page" ref={pageRootRef}>
  ...
  <div className={`operation-hierarchy-layout operation-hierarchy ${navigatorCollapsed?"navigator-collapsed":"navigator-expanded"}`}>
   <div className="operation-tree-pane artifact-navigator operation-category-enabled">
    <div className="artifact-navigator-content categorized-operation-content" ref={treeRef}><div className="artifact-tree-chip-title"><span>TREE</span><select aria-label="Tree parent-child relationship" value={visibilityRules.relationshipMode} onChange={event=>setVisibilityRules({...visibilityRules,relationshipMode:event.target.value as TreeRelationshipMode})}><option value="implementation">IMPLEMENTS ↔ IMPLEMENTED BY</option><option value="inheritance">INHERITS FROM ↔ INHERITED BY</option><option value="dependency">DEPENDS ON ↔ DEPENDED ON BY</option></select></div>{visibilityRules.relationshipMode==="implementation"?<CategorizedArtifactTree items={categorizedOperations} onlyCategories={false} categoryCommand={categoryCommand} workspaceId={workspaceId} categoryTree="operations"/>:relationshipForest(visibilityRules.relationshipMode)}</div>
    ...
    {viewControlsOpen&&<TreeViewControls kinds={treeKinds} rules={visibilityRules} onChange={setVisibilityRules} showParents={showParents} onShowParentsChange={setShowParents} onBranchAction={commandBranches} relationshipModesEnabled/>}
   </div>
