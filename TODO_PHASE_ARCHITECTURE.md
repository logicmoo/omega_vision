...
Location: `python/arc_cli_debugger/arc3_runner.py`
...
Location: `python/arc_cli_debugger/action_tree.py`
...
### `python/arc_cli_debugger/config/llm_providers.json`
...
Location: `python/arc_cli_debugger/llm_providers.py`
...
Location: `python/arc_cli_debugger/unsloth_studio.py`
...
Location: `python/omega_vision/gpt_bridge.py`
...
- `python/arc_cli_debugger/llm_json.py` — strict parsing, local repair, required-key validation, and normalized serialization.
- `python/arc_cli_debugger/llm_json_patch.py` — records initial output, performs local repair or one text-only repair request, and keeps the interactions in one transcript.

### `LlmTranscriptRun`

Location: `python/arc_cli_debugger/llm_transcripts.py`
...
`python/arc_cli_debugger/llm_readme_patch.py` marks the active completed transcript, links every historical run, and prevents recursive transcript embedding.
...
- resolve `python/arc_cli_debugger/config/llm_providers.json` independently;
...
### `python/arc_cli_debugger/project_paths.py`
...
`GenerativeForm` is the exact-grid generative form backed by `prolog/omega_vision/turtle_dsl.pl`.
...
- generated `python/notebooks/submission.ipynb`;
